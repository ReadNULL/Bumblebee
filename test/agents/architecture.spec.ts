...
const AGENTS_ROOT = path.resolve("src/agents");

describe("agents architecture", () => {
  it("keeps agent contracts independent from pi, runtime, and integrations", async () => {
    const violations: string[] = [];

    for (const sourceFile of await collectTypeScriptFiles(AGENTS_ROOT)) {
      const relative = normalizePath(path.relative(AGENTS_ROOT, sourceFile));
  ...
  it("publishes every agent source and excludes tests", async () => {
    ...
    const missing = (await collectTypeScriptFiles(AGENTS_ROOT))
