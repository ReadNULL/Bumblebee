...
`Bumblebee Composite Score v1`。它不执行任务、不调用模型；职责仅限于校验来源、
判定资格、计算冻结权重和生成可审计报告。当前支持两种明确区分的发布入口：

- `score`：四个标准 run 的严格单次运行口径；
- `publish`：测试基础设施反复中断时使用的项目环境恢复聚合口径。

第二种口径可以正式发布 Bumblebee 项目分数，但必须同时说明它跨运行、跨 commit，
不能称为上游官方排行榜或单一版本的一次运行。
...
Terminal-Bench 当前以 `environment-recovery-aggregate` 正式发布项目
`TB = 93.33`。该口径源于 Docker、PyPI 和模型服务异常造成的多次恢复运行：
每个任务只能选择一整个 5-trial 批次，不能逐 trial 挑选；两个 infrastructure
invalid 仍按 0 计入固定 45 次分母。严格 `score` 入口仍拒绝把该聚合伪装成单个
Benchmark 0 run。
...
命令会输出 `TB = 93.33`、`BCS-v1 = 96.65` 和全局有效任务率 `99.81%`。
...
```mermaid
flowchart TD
  Jobs["保留 6 次 TB job 原始结果"] --> Hashes["冻结 job ID、commit 与产物 SHA-256"]
  Hashes --> Batch["每个任务只选择完整 5-trial 批次"]
  Batch --> Invalid["基础设施 invalid 必须说明原因并按 0 计分"]
  Invalid --> Coverage["校验冻结 9 题恰好覆盖一次"]
  Coverage --> Sources["绑定 BB / AD / LM 合格标准 run"]
  Sources --> Gates["执行 BCS-v1 全局硬门槛"]
  Gates --> Score["发布项目 TB 与 BCS-v1"]
```

清单保存所有候选 job 总计，不会把失败或 invalid 从历史中删除。聚合批次的统计不能
超过对应 job 原始统计；AD 分数还会从三个冻结指标重新推导，防止手工改分。
...
本次环境恢复发布为 `qualified`。这里的 TB 资格是 Bumblebee 项目聚合协议资格；
各次原始 TB job 的 `invalid` 或局部覆盖状态保持不变。
...
缺失门槛、Markdown、端到端写入和环境恢复发布测试。正式发布结果见
[`PUBLISHED_2026-07-25.md`](./PUBLISHED_2026-07-25.md)：
`TB = 93.33`，`BCS-v1 = 96.65`。
